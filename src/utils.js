...
const createEditIcon = (container, size) => {
  ...
  const g1 = svg.append('g').attr('id', 'Complete');

  const g2 = g1.append('g').attr('id', 'edit');
...
const createTrashIcon = (container, size) => {
  ...
  const g1 = svg.append('g').attr('id', 'SVGRepo_iconCarrier');
...
const createCancelIcon = (container) => {
  ...
  outerGroup.append('circle').attr('cx', 8.5).attr('cy', 8.5).attr('r', 8);

  const innerGroup = outerGroup
    .append('g')
    .attr('transform', 'matrix(0 1 -1 0 17 0)');

  innerGroup.append('path').attr('d', 'm5.5 11.5 6-6');

  innerGroup.append('path').attr('d', 'm5.5 5.5 6 6');
};

const addHoverEffect = (element, svg, property) => {
  // Change icon color to red when hovered over
  element.addEventListener('mouseover', () =>
    svg.style.setProperty(property, 'red')
  );
  // Change it back to standard
  element.addEventListener('mouseout', () =>
    svg.style.setProperty(property, '#000000')
  );
};
